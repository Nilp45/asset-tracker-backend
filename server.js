...
function auth(req, res, next) {
  ...
  if (!hdr) return res.status(401).json({ error: "Session expired" });
...
app.post("/api/scan", auth, async (req, res) => {
  ...
  /* 🚫 HARD RULE: OUT cannot exceed qty */
  if (
    session.mode === "OUT" &&
    session.totalQty &&
    ...
    return res.status(400).json({
      error: "OUT quantity already completed"
    });
  }
  ...
  /* ✅ OUT auto-complete ONLY on exact qty */
  if (
    session.mode === "OUT" &&
    session.totalQty &&
    session.scannedQty === session.totalQty
